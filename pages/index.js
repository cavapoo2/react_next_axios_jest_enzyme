...
export default class CodeForm extends Component {
  ...
  validateInput(value) {
    //an extra space is ok!
    function contains(data, anyOf) {
      for (let i = 0; i < data.length; i++) {
        for (let j = 0; j < anyOf.length; j++) {
          if (data[i].includes(anyOf[j], 0)) {
            return true;
          }
        }
      }
    ...
    let data = value.split(" ").filter(x => x.length > 0)
    let anyNonValidNumbers = data.filter(x => isNaN(x) || contains(x, ['.', '-']))
    if (anyNonValidNumbers.length > 0 || data.length === 0) {
      this.setState({ valid: false })
    } else {

      this.setState({ valid: true, data: data })
    }
